...
consumer = KafkaConn(os.getenv('TOPIC_2'))
...
def pull_data():
    try:
        ...
        mongodb_client.save_data(os.getenv('TOPIC_2'), messages)
...
def get_data():
    try:
        cursor = mongodb_client.get_all_data(os.getenv('TOPIC_2'))
        list_result = []
        for doc in cursor:
           list_result.append(doc)
        return {"responce" : f"get data from {os.getenv('TOPIC_2')} collection went seccessfully.", "data" : list_result}
    except Exception as e:
